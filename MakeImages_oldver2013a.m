function MakeImages(clusmap_data,FileName,RegionBounds,ProcSet)
    ...
    if ProcSet.SavePointPlots
        ...
        figure('Color','w', 'Visible', 'off', 'Renderer', 'OpenGL', 'Units', 'pixels'); %painters?
        set(gca,'Units','inches','DataAspectRatio', [1,1,1],'Position', [1 1 10 10]);    
        box('off');
        hold('on');
        set(gcf, 'PaperUnits', 'inches', 'PaperSize', [12 12], 'PaperPositionMode', 'manual', 'PaperPosition', [0 0 12 12],'Visible','off');
        colormap(ProcSet.CustomColorMap);
        
        scatter(clusmap_data(:,1), clusmap_data(:,2),4,'o','fill');
        axis(RegionBounds);
    ...
    if ProcSet.SaveGFPlots
        ...
        scatter(clusmap_data(:,1), clusmap_data(:,2), 12, clusmap_data(:,3),'filled');
        caxis([0 ProcSet.ColMapMax]);
        axis(RegionBounds);
        axis square image tight
        set(gca, 'Visible','off');
