...
prompt = {'CSR Repeats:','Do G&F Stuff? (0=randomise xy only)','Do images? (0=faster, no images)'};
dlg_title = 'Enter the settings to use...';
num_lines = 1;
def = {num2str(CSRRepeats),num2str(DoGFStuff),num2str(DoImages)};
answer = inputdlg(prompt,dlg_title,num_lines,def);

if ~isempty(answer)
    CSRRepeats = str2double(answer(1,1));
    DoGFStuff = str2double(answer(2,1));
    DoImages = str2double(answer(3,1));
else
error('Cancelled?! So rude.');
end
...
%         axis image
    ...
    %% do the regular interpolated maps
        ...
        %% do the grid-maps version
            if ProcSettings.DoGridMaps && DoImages
    ...
    %             SavePNG('GF-Points','GFPoints_GRID',ProcSettings.UseFolders); 
            ...
            if ProcSettings.DoClustersByBlobs && DoImages
