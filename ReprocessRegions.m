...
dirName = uigetdir(pwd,'Choose your data folder');
...
if dirName ~=0
    ...
    if exist(fullfile(cd, 'ProcSettings.txt'), 'file') == 0
        errordlg('Cannot find ''ProcSettings.txt'' file in this folder. Select the appropriate one for your data in the next window.');

        [~,dirName2] = uigetfile({'ProcSettings.txt*';'*.*'},'Choose ProcSettings file that matches your data.');
        cd(dirName2);
...
% We have files to remangle so lets have somewhere to save them
